...
struct LocalBusDispatchResult {
    ...
    int load_changes{};
    int shunt_block_changes{};
    double shunt_polish_seconds{};
    double predicted_native_gain{};
    bool changed() const {
        return active_generation_changes + reactive_generation_changes + load_changes + shunt_block_changes > 0;
    }
};
...
public:
    ...
    LocalBusDispatchResult improve(
        ...
        const std::vector<double>& network_q, AcState& candidate, int passes = 2,
        bool polish_discrete_shunts = false) const;
private:
...
// Candidate generation only. Keep voltages/angles/branch flows fixed, and use
...
// Optional endpoint shunt polishing changes only authorized integer blocks;
// the caller must update nodal injections for their changed susceptances.
LocalBusDispatchResult improve_fixed_network_bus_dispatch(
    ...
    AcState& candidate, int passes = 2, bool polish_discrete_shunts = false);
